...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
  ...
  return (
    <div className="flex items-center justify-end space-x-2 py-4">
      ...
      {pageNumbers
        .slice(Math.max(0, currentPage - 2), Math.min(totalPages, currentPage + 5))
        .map((page) => (
          <Button
            key={page}
            variant="outline"
            ...
            className={currentPage === page ? "w-8 rounded-md shadow-md bg-[#777777] border-[#777777] font-sans text-sm font-bold leading-6 text-white text-left" : ""}
          >
            {page + 1}
          </Button>
      ))}
